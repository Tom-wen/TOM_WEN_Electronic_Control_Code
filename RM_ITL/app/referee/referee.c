...
{
    //常规链路(使用机械臂模块时不编译，会与接收自定义控制器数据冲突)
    #ifndef COMPILE_ARM
...
{
    float power = chassis_total_power;
    
    // 限幅 0~120W
    if (power < 0.0f) power = 0.0f;
    if (power > 120.0f) power = 120.0f;
    
    // 线性映射: 0W → angle 232, 120W → angle 270
    ui_static_ui_group4_power->start_angle = 232;
    ui_static_ui_group4_power->end_angle = (int16_t)(233.0f + (power / 120.0f) * 38.0f);
...
{
    if(shoot_cmd_send.shoot_mode == SHOOT_NORMAL){
