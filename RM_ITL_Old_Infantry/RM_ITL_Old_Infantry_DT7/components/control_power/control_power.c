...
void MotorPower_Init(MotorPower* motor, MotorInstance *motors, uint8_t motor_number)
{
    for(int i = 0; i < motor_number; i++)
    {
        motor[i].current_current = motors[i].motor_data->feedback->current;
        motor[i].current_speed = motors[i].motor_data->feedback->vel / 9.5541f;        

    }
}

//总目标实现功率控制
void Chassis_Power_Total_Control(float total_power_limit, uint8_t motor3508_number, uint8_t motor6020_number, MotorPower* motor) { 
...
float Chassis_3508_Get_Limited_Current(uint8_t motor_id) {
    if (motor_id >= 4) return 0.0f;
    return chassis_3508_motor[motor_id].decay_number;
}

float Chassis_6020_Get_Limited_Current(uint8_t motor_id) {
    if (motor_id >= 4) return 0.0f;
...
double motor_power_update(MotorPower* motor, E_Predict_Status_Type predict_status,
    ...
    // 转换并取绝对值
    motor->current_current = fabs(motor_power_get_real_current(motor, motor->current_current));
    motor->current_speed = fabs(motor->current_speed);
    // 计算功率值
    float power = (motor->k0 + motor->k1*motor->current_current + motor->k2*motor->current_speed + 
        motor->k3 * motor->current_current * motor->current_speed + motor->k4 * motor->current_current * motor->current_current 
        + motor->k5 * motor->current_speed * motor->current_speed) * power_sign;
    // 根据预测状态存储功率值到相应字段
...
void Total_6020motor_Power_Control(float total_power_limit, uint8_t motor6020_number, uint8_t motor3508_number){
    ...
    for (int j = 0; j < motor6020_number; j++){
        ...
        chassis_6020_motor[j].decay_number = (&chassis_6020_motor[j], motor6020_number);
    }
}
