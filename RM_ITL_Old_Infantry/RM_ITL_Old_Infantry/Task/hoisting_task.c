...
void hoisting_task(void const *pvParameters)
{
  ...
  hoisting_init();
  Upper_Computer_Init(&hoisting_motor[3].motor_gyro_set);
  Upper_Computer_Init(&hoisting_motor[3].motor_gyro);
  while(1)
  {
...
static void hoisting_init(void)
{
  ...
  for (int i = 0; i < 4; i++)
  {
    PID_init(&hoisting_motor[i].hoisting_motor_relative_pid,HOISTING_MOTOR_RELATIVE_KP,HOISTING_MOTOR_RELATIVE_KI,HOISTING_MOTOR_RELATIVE_KD,HOISTING_MOTOR_RELATIVE_MAX_OUT,HOISTING_MOTOR_RELATIVE_MAX_IOUT);
    PID_init(&hoisting_motor[i].hoisting_motor_gyro_pid,HOISTING_MOTOR_GYRO_KP,HOISTING_MOTOR_GYRO_KI,HOISTING_MOTOR_GYRO_KD,HOISTING_MOTOR_GYRO_MAX_OUT,HOISTING_MOTOR_GYRO_MAX_IOUT);
  }
}
