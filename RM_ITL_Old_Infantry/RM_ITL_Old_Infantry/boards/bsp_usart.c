...
{  
    ...
    else if(huart == &huart10)
    {
        ...
        HAL_UARTEx_ReceiveToIdle_DMA(&huart10, uart10_rx_data_t[uart10_buff_ctrl].buffer, UART_BUFFER_SIZE);
    }
}
...
void HAL_UARTEx_RxEventCallback(UART_HandleTypeDef *huart, uint16_t Size)
{
    ...
    else if(huart->Instance == UART5)
    {
        // 更新接收时间戳
            ...
            sbus_online = 1;
        #else
                // 如果不使用RTOS，可以使用其他方式获取时间戳，或者保持last_sbus_recv_time不变
                // 例如: last_sbus_recv_time = get_system_tick();
        #endif
    ...
    {
        usart_rx_dma_start(huart, usart7_rx_buf, sizeof(usart7_rx_buf));
    }
    // ... 其它串口同理
    else if(huart->Instance == USART10) // 判断是否是空闲中断
    {
        ...
        uart10_rx_data_t[uart10_buff_ctrl].size = UART_BUFFER_SIZE - __HAL_DMA_GET_COUNTER(&hdma_usart10_rx);
        /* 将这个串口数据的结构体地址给指针 */
        pUartData = &uart10_rx_data_t[uart10_buff_ctrl];
        /* 把指向串口接收数据的指针放入消息队列。注意，这里传的是指针的地址，也就是指针串口数据结构体的指针的地址不能传pUartData本身 */
        /* 这里进行加一，使其下一次DMA传输时将接收的数据放到下一个串口缓冲区中 */
        uart10_buff_ctrl++;
        /* 取余操作防止越界 */
        uart10_buff_ctrl %= UART_BUFFER_QUANTITY;
        /* 重启开始DMA传输 */
        usart_rx_dma_start(&huart10, uart10_rx_data_t[uart10_buff_ctrl].buffer, UART_BUFFER_SIZE);  // USART10 DMA接收初始化
    }
