...
uint16_t gq_motor_id_map[MAX_GQ_MOTORS] = {0x100, 0x200, 0x300, 0x400, 0x500, 0x600, 0x700, 0x800, 0x900, 0xA00, 0xB00};
//高擎电机初始化
void GQ_Motor_Init(MotorInstance *motors)
{
    for(int i = 0; i < motors[0].motor_count; i ++)
    {
        timed_return_motor_status(motors[i].motor_data->hfdcan, motors[i].motor_data->id, 1);
        DWT_Delay(0.01);
    }
}

//高擎电机使能
void GQ_Motor_enable(MotorControlData *motors)
{
    motors->motor_enable = 1;
}
//高擎电机失能（标志位）
void GQ_Motor_disable(MotorControlData *motors)
{
 ...
 * @brief 电流模式
 * @param id 电机ID
 * @param cur 电流值：单位根据具体电机规格确定
 */
void GQ_Motor_Current_mode(MotorInstance *motors)
{
    uint8_t data[8] = {0x01, 0x00, 0x09, 0x05, 0x1c, 0x00, 0x00, 0x00};				//发送数据
    for(int i = 0; i < motors[0].motor_count; i ++)
    {
        if(motors[i].motor_data->motor_enable != 0)
        {
            *(int16_t *)&data[5] = motors[i].motor_data->target_current;
            fdcanx_send_data(motors[i].motor_data->hfdcan, (0x8000 | motors[i].motor_data->id), data, 8);
        }
        else 
        {      
