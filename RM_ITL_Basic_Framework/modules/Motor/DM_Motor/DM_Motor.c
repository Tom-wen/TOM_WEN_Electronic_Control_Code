...
{
    if(motors[0].motor_data->feedback->tor == 0){
        DM_Motor_Init(motors);
    }
}

//DM电机数据处理
void DM_Motor_TimeoutCheck(CAN_PORT can_port, uint16_t threshold)
{
    if(can_port >= MAX_CAN) return;

    for(int i = 0; i < MAX_DM_MOTORS; i++)
    {
        DM_Motor_RX[can_port][i].lost_cnt++;
        if(DM_Motor_RX[can_port][i].lost_cnt >= threshold)
        {
            DM_Motor_RX[can_port][i].p_int = 0;
            DM_Motor_RX[can_port][i].v_int = 0;
            DM_Motor_RX[can_port][i].t_int = 0;
            DM_Motor_RX[can_port][i].pos   = 0.0f;
            DM_Motor_RX[can_port][i].vel   = 0.0f;
            DM_Motor_RX[can_port][i].tor   = 0.0f;
            DM_Motor_RX[can_port][i].temp  = 0.0f;
            DM_Motor_RX[can_port][i].current = 0.0f;
        }
    }
}
...
void DM_motor_can_callback(CANRxData *Rx_data, CAN_PORT can_port)
{
    ...
    if(found)
    {
        ...
        DM_Motor_RX[can_port][motor_index].lost_cnt = 0;  // 收到CAN数据，清除丢失计数
    }
    else
    {
        // CAN ID未匹配（非DM电机消息），对所有DM电机执行超时检查
        DM_Motor_TimeoutCheck(can_port, DM_LOST_THRESHOLD);
    }
