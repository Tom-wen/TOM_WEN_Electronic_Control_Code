#include "uav_gimbal.h"
#ifdef COMPILE_UAV_GIMBAL

// ============================================================================
// 全局变量定义
// ============================================================================

/**
 * @brief 云台控制命令结构体
 * @details 用于存储云台控制的各种参数和命令
 */
Gimbal_Ctrl_Cmd_s gimbal_cmd_send;
/**
 * @brief 云台测量数据结构体
 * @details 用于存储云台当前的状态测量数据，传输给视觉系统
 */
Gimbal_measure gimbal_measure_data;

/**
 * @brief 自瞄开启关闭标志位
 * @details 1表示开启自瞄模式，0表示关闭自瞄模式
 */
uint8_t auto_aim_flag = 0;
// 静态变量：记录上一次有效的目标角度
static float yaw_target_last = 0;
static float pitch_target_last = 0;
float pitch_angle = 0.0f;
float yaw_angle = 0.0f;
// ============================================================================
// 任务函数实现
// ============================================================================

/**
 * @brief 云台控制任务函数
 * @param[in] argument 任务参数指针
 * @details 这是云台控制的主任务函数，在FreeRTOS中周期性运行
 *          负责处理云台模式切换、控制逻辑执行和电机状态更新
 */
void gimbal_task(void *argument)
{
    static gimbal_mode_e last_mode = GIMBAL_ZERO_FORCE;
    // 初始化云台电机系统
    Gimbal_Init(Gimbal_6020);
    // 无限循环，持续执行云台控制逻辑
    for(;;)
    {
        pitch_angle = Get_Pitch_Angle_From_Encoder(Gimbal_6020);
        yaw_angle = Get_Yaw_Angle_From_Encoder(Gimbal_6020);
        // 处理遥控器输入数据，映射到云台控制命令
        RemoteControlGimbal(&gimbal_cmd_send);
        // 更新云台控制模式（根据遥控器拨杆状态）
        Gimbal_mode_update(&gimbal_cmd_send);
        //PID数据切换
        Gimbal_PID_Change(Gimbal_6020);
        // 检测模式切换，清除PID积分器以避免突变
        if(last_mode == GIMBAL_ZERO_FORCE && gimbal_cmd_send.gimbal_mode != GIMBAL_ZERO_FORCE)
        {
            Gimbal_PIDClear(Gimbal_6020);
        }
        last_mode = gimbal_cmd_send.gimbal_mode;
        // 根据当前云台模式执行相应的控制逻辑
        switch (gimbal_cmd_send.gimbal_mode)
        {
            case GIMBAL_ZERO_FORCE:
                // 零电流模式：关闭电机使能，电机不输出力矩
                Gimbal_Motor_Status(Motor_Disable, Gimbal_6020, 2);
                break;
                
            case GIMBAL_NORMAL:
                // 正常模式：使能电机并执行回零点操作
                Gimbal_Motor_Status(Motor_Enable, Gimbal_6020, 2);
                Gimbal_control(Gimbal_6020, &gimbal_cmd_send, &vision_feedback, &INS);
                //Gimbal_Measure(&gimbal_measure_data, &INS, Gimbal_6020);
                break;
                
            case GIMBAL_ABSOLUTE_ANGLE:
                // 绝对角度模式：使能电机并执行云台控制
                Gimbal_Motor_Status(Motor_Enable, Gimbal_6020, 2);
                Gimbal_control(Gimbal_6020, &gimbal_cmd_send, &vision_feedback, &INS);
                // 注释掉的代码：数据测量功能
                //Gimbal_Measure(&gimbal_measure_data, &INS, Gimbal_6020);
                break;
        }
        //上位机调PID用
        //PID_Test_Init(PID_K, Gimbal_6020);
       
        // 更新云台电机控制（发送控制指令给电机）
        gimbal_motor_updata(Gimbal_6020);
        
        // 任务延时2ms，控制任务执行频率
        vTaskDelay(pdMS_TO_TICKS(2));
    }
}

// ============================================================================
// 初始化函数实现
// ============================================================================

/**
 * @brief 云台初始化函数
 * @param[in] motors 电机实例数组指针
 * @details 初始化云台电机的PID控制器参数
 *          包括单环PID、串级PID的内环和外环
 */
void Gimbal_Init(MotorInstance *motors)
{
    // YAW轴（偏航轴）6020电机PID初始化
    PID_Init(&motors[0].motor_data->pid[single_loop], 1.0f, 0.0f, 0.0f, 10.0f, 6000.0f, 0.0f);
    PID_Init(&motors[0].motor_data->pid[cascade_inner], 90.0f, 0.0f, 0.0f, 600.0f, 12000.0f, 0.0f);
    PID_Init(&motors[0].motor_data->pid[cascade_outer], 10.0f, 0.0f, 0.0f, 10.0f, 90.0f, 0.0f);
        
    // PITCH轴（俯仰轴）6020电机PID初始化
    PID_Init(&motors[1].motor_data->pid[single_loop], 1.0f, 0.0f, 0.0f, 10.0f, 6000.0f, 0.0f);
    PID_Init(&motors[1].motor_data->pid[cascade_inner], 200.0f, 0.0f, 0.0f, 100.0f, 9000.0f, 0.0f);
    PID_Init(&motors[1].motor_data->pid[cascade_outer], 3.0f, 0.0f, 0.3f, 10.0f, 40.0f, 0.0f);
}
/**
 * @brief 清除云台PID控制器积分项
 * @param motors 云台电机实例数组指针
 * @details 在模式切换时调用，防止PID积分累积导致冲击
 */
void Gimbal_PIDClear(MotorInstance  *motors)
{
    if(motors == NULL)
    {
        return;
    }
    //云台6020
    for(int i = 0;i < 2; i++)
    {
        PID_Clear(motors[i].motor_data->pid);
    }
}

// ============================================================================
// 电机控制函数实现
// ============================================================================

/**
 * @brief 云台电机状态设置函数
 * @param[in] status 电机状态（使能或失能）
 * @param[in] motors 电机实例数组指针
 * @param[in] motor_count 电机数量
 * @details 根据指定状态批量设置云台电机的使能状态
 */
void Gimbal_Motor_Status(Motor_status status, MotorInstance *motors, uint8_t motor_count)
{
    switch (status)
    {
        case Motor_Enable:
            // 使能所有指定的电机
            for(int i = 0; i < motor_count; i++)
            {
                DJI_Motor_enable(motors[i].motor_data);
            }
            break;
            
        case Motor_Disable:
            // 失能所有指定的电机
            for(int i = 0; i < motor_count; i++)
            {
                DJI_Motor_disable(motors[i].motor_data);
            }
            break;
            
        default:
            // 默认情况下失能所有电机
            for(int i = 0; i < motor_count; i++)
            {
                DJI_Motor_disable(motors[i].motor_data);
            }
            break;
    }
}

// ============================================================================
// 模式控制函数实现
// ============================================================================

/**
 * @brief 云台模式更新函数
 * @param[in,out] Gimbal_Cmd 云台控制命令结构体指针
 * @details 根据遥控器左拨杆状态更新云台控制模式
 */
void Gimbal_mode_update(Gimbal_Ctrl_Cmd_s *Gimbal_Cmd)
{
    // 参数有效性检查
    if(Gimbal_Cmd == NULL)
    {
        return;
    }
    
    // 检查SBUS遥控器是否在线
    if(sbus_online == 0)
    {
        // 遥控器不在线时设置为零电流模式
        Gimbal_Cmd->gimbal_mode = GIMBAL_ZERO_FORCE;
        return;
    }
    
    #ifdef DJI_REMOTE
    // 根据左拨杆位置设置云台模式
    switch (left_switch)
    {
        case switch_down:
            // 拨杆向下：零电流模式
            Gimbal_Cmd->gimbal_mode = GIMBAL_ZERO_FORCE;
            break; 
            
        case switch_mid:
            // 拨杆中间：正常模式
            Gimbal_Cmd->gimbal_mode = GIMBAL_NORMAL;
            break;
            
        case switch_up:
            // 拨杆向上：绝对角度模式
            Gimbal_Cmd->gimbal_mode = GIMBAL_ABSOLUTE_ANGLE;
            break;    
            
        default:
            break;
    }
    #endif
    #ifdef FS_REMOTE
    // 根据左拨杆位置设置云台模式
    switch (right_switch)
    {
        case switch_up:
            // 拨杆向下：零电流模式
            Gimbal_Cmd->gimbal_mode = GIMBAL_ZERO_FORCE;
            break; 
            
        case switch_mid:
            // 拨杆中间：正常模式
            Gimbal_Cmd->gimbal_mode = GIMBAL_NORMAL;
            break;
            
        case switch_down:
            // 拨杆向上：绝对角度模式
            Gimbal_Cmd->gimbal_mode = GIMBAL_ABSOLUTE_ANGLE;
            break;    
            
        default:
            break;
    }
    #endif
}

/**
 * @brief 遥控器数据映射函数
 * @param[in,out] Gimbal_Cmd 云台控制命令结构体指针
 * @details 将遥控器通道数据映射到云台控制参数
 */
void RemoteControlGimbal(Gimbal_Ctrl_Cmd_s *Gimbal_Cmd) 
{
    // 参数有效性检查
    if(Gimbal_Cmd == NULL)
    {
        return;
    }
    
    // 根据当前云台模式映射遥控器数据到云台控制参数
    switch(Gimbal_Cmd->gimbal_mode) 
    {
        case GIMBAL_NORMAL:     
            // 正常模式：直接映射遥控器摇杆数据
            Gimbal_Cmd->yaw = -(float)rc_ctrl.rc.ch[2];     // YAW轴控制（左右旋转）
            Gimbal_Cmd->pitch = (float)rc_ctrl.rc.ch[3];    // PITCH轴控制（上下俯仰）
            Gimbal_Cmd->yaw_vel = 0;                        // YAW轴速度设为0
            Gimbal_Cmd->pitch_vel = 0;                      // PITCH轴速度设为0
            break;
            
        case GIMBAL_ABSOLUTE_ANGLE:             
            // 绝对角度模式：同样映射遥控器摇杆数据
            Gimbal_Cmd->yaw = -(float)rc_ctrl.rc.ch[2];     // YAW轴控制
            Gimbal_Cmd->pitch = (float)rc_ctrl.rc.ch[3];    // PITCH轴控制
            Gimbal_Cmd->yaw_vel = 0;                        // YAW轴速度设为0
            Gimbal_Cmd->pitch_vel = 0;                      // PITCH轴速度设为0
            break;
            
        case GIMBAL_ZERO_FORCE:     
            // 零电流模式：所有控制参数设为0
            Gimbal_Cmd->yaw = 0;        // YAW轴目标值为0
            Gimbal_Cmd->pitch = 0;      // PITCH轴目标值为0
            Gimbal_Cmd->yaw_vel = 0;    // YAW轴速度为0
            Gimbal_Cmd->pitch_vel = 0;  // PITCH轴速度为0
            break;
            
        default:
            break;
    }
}

// ============================================================================
// 电机更新函数实现
// ============================================================================

/**
 * @brief 云台电机状态更新函数
 * @param[in] motors 电机实例数组指针
 * @details 调用电机控制函数，将控制指令发送给电机
 */
void gimbal_motor_updata(MotorInstance *motors)
{
    // 参数有效性检查
    if(motors == NULL)
    {
        return;
    }
    
    // 调用电机控制函数，更新电机状态
    motors->motor_control(motors);
}

// ============================================================================
// 主控制函数实现
// ============================================================================

/**
 * @brief 云台控制主函数
 * @param[in] motors 电机实例数组指针
 * @param[in,out] Gimbal_Cmd 云台控制命令结构体指针
 * @param[in] vision_feedback 视觉反馈数据指针
 * @param[in] ins 惯性导航系统数据指针
 * @details 根据自瞄标志位选择控制模式（自瞄或手动）
 */
void Gimbal_control(MotorInstance *motors, Gimbal_Ctrl_Cmd_s *Gimbal_Cmd, VisionToGimbal *vision_feedback, INS_t *ins)
{
    #ifdef DJI_REMOTE
    // 根据侧拨杆状态更新自瞄标志位
    if(side_switch_on)
    {
        // 侧拨杆打开：开启自瞄模式
        auto_aim_flag = 1;
    }
    else if(side_switch_off)
    {
        // 侧拨杆关闭：关闭自瞄模式
        auto_aim_flag = 0;
    }
    #endif

    #ifdef FS_REMOTE
    switch (left_switch)
    {
    case switch_up:
        // 侧拨杆关闭：关闭自瞄模式
        auto_aim_flag = 0;
        break;
    case switch_down:
        // 侧拨杆打开：开启自瞄模式
        auto_aim_flag = 1;
        break;
    default:
        // 侧拨杆关闭：关闭自瞄模式
        auto_aim_flag = 0;
        break;
    }
    #endif
    
    // 根据自瞄标志位选择控制模式
    switch (auto_aim_flag)
    {
        case AUTO_AIM_ON:
            if(vision_feedback->mode == 1 || vision_feedback->mode == 2)
            {
                // 自瞄模式开启：执行自动瞄准计算
                Auto_aiming_cal(motors, Gimbal_Cmd, vision_feedback, ins);
            }
            break;           
        case AUTO_AIM_OFF:
            // 自瞄模式关闭：执行遥控器手动控制
            remote_control(motors, Gimbal_Cmd, &INS);
            break;
            
        default:
            break;
    }
}

// ============================================================================
// 手动控制函数实现
// ============================================================================

/**
 * @brief 遥控器控制云台运动函数
 * @param[in] motors 电机实例数组指针
 * @param[in,out] Gimbal_Cmd 云台控制命令结构体指针
 * @param[in] ins 惯性导航系统数据指针
 * @details 实现基于遥控器输入的云台手动控制，包含PID控制和机械限位保护
 */
    static float yaw_left_limit = 0; 
    static float yaw_right_limit = 0;
void remote_control(MotorInstance *motors, Gimbal_Ctrl_Cmd_s *Gimbal_Cmd, INS_t *ins)
{
    // YAW轴控制（偏航轴，左右旋转）
    float yaw_current = yaw_angle;  // 当前陀螺仪角度
    static float yaw_target = 0;    // YAW目标角度
    static uint8_t yaw_first_run = 1;

    if(yaw_first_run == 1)
    {
        yaw_target = yaw_current;  // 初始目标为当前角度（0度）

        // 读取当前YAW电机编码器位置
        uint16_t current_encoder = motors[0].motor_data->feedback->pos;

        // 计算当前位置离左右限位的编码器刻度差
        int16_t left_space = current_encoder - yaw_encoder_left;   // 离左限位的空间(正数)
        int16_t right_space = yaw_encoder_right - current_encoder; // 离右限位的空间(正数)

        // 如果你的限位范围不跨越0点，可以不用处理

        // 将编码器刻度转换为角度（GM6020是8192刻度/360度）
        float left_angle = (float)left_space * 360.0f / 8192.0f;   // 可以向左转的角度
        float right_angle = (float)right_space * 360.0f / 8192.0f; // 可以向右转的角度

        // 基于当前陀螺仪位置（0度）设置限位
        yaw_left_limit = left_angle;   // 负值表示向左
        yaw_right_limit = -right_angle;   // 正值表示向右

        yaw_first_run = 0;
    }

    // 根据遥控器输入更新YAW目标角度
    yaw_target += Gimbal_Cmd->yaw * yaw_sensitivity;
    // 动态限位保护
    if(yaw_target < yaw_right_limit)
    {
        yaw_target = yaw_right_limit;
    }
    else if(yaw_target > yaw_left_limit)
    {
        yaw_target = yaw_left_limit;
    }

    // 执行YAW轴串级PID控制计算
    PID_CascadeCalc(motors[0].motor_data->pid, yaw_current, yaw_target, motors[0].motor_data->feedback->vel);
    motors[0].motor_data->target_position = motors[0].motor_data->pid[cascade_inner].Out;
    // PITCH轴控制（俯仰轴，上下运动）
    // 定义机械限位范围
    const float pitch_max_limit = pitch_max;    // PITCH轴最大值（向上限位）
    const float pitch_min_limit = pitch_min;    // PITCH轴最小值（向下限位）

    // 使用陀螺仪PITCH角度作为反馈值
    //float pitch_current = ins->Pitch;//pitch_angle;           // 获取当前PITCH角度
    float pitch_current = pitch_angle;
    static float pitch_target = 0;   // PITCH目标角度，初始为偏移量
    static uint8_t pitch_first_run = 1;
    if(pitch_first_run == 1)
    {
        pitch_target = pitch_current;
        pitch_first_run = 0;
    } 
    // 根据遥控器输入更新PITCH目标角度
    pitch_target += Gimbal_Cmd->pitch * pitch_sensitivity;  
    //pitch_target = 10;
    // 机械限位保护：限制目标角度在允许范围内
    if (pitch_target > pitch_min_limit) 
    {
        // 超过下限则限制在下限值`
        pitch_target = pitch_min_limit;
    }
    else if (pitch_target < pitch_max_limit)
    {
        // 超过上限则限制在上限值
        pitch_target = pitch_max_limit;
    }

    // 执行PITCH轴串级PID控制计算
    // 参数：PID控制器、目标角度、当前角度、前馈值（X轴角速度）
    PID_CascadeCalc(motors[1].motor_data->pid, pitch_target, pitch_current, motors[1].motor_data->feedback->vel);
    // 设置PITCH轴电机目标位置
    motors[1].motor_data->target_position = motors[1].motor_data->pid[cascade_inner].Out;// + Gravity_compensation(ins->Pitch) * current_change;

}

// ============================================================================
// 自瞄控制函数实现
// ============================================================================

/**
 * @brief 自瞄解算函数
 * @param[in] motors 电机实例数组指针
 * @param[in,out] Gimbal_Cmd 云台控制命令结构体指针
 * @param[in] vision_feedback 视觉反馈数据指针
 * @param[in] ins 惯性导航系统数据指针
 * @details 处理视觉系统反馈的数据，实现自动瞄准功能
 */
void Auto_aiming_cal(MotorInstance *motors, Gimbal_Ctrl_Cmd_s *Gimbal_Cmd, VisionToGimbal *vision_feedback, INS_t *ins)
{
    // ========== YAW轴控制 ==========
    static uint8_t first_run = 1;
    static float yaw_left_limit = 0; 
    static float yaw_right_limit = 0; 

    // YAW轴自瞄控制（陀螺仪绝对角度闭环）
    float yaw_current = yaw_angle;  // 当前陀螺仪YAW角度（-180到180）
    // 确定目标角度
    float yaw_target;
    // 首次运行时，初始化限位范围
    if (first_run)
    {
        yaw_target = yaw_current;
        // 读取当前YAW电机编码器位置
        uint16_t current_encoder = motors[0].motor_data->feedback->pos;

        // 计算当前位置离左右限位的编码器刻度差
        int16_t left_space = current_encoder - yaw_encoder_left;   // 离左限位的空间
        int16_t right_space = yaw_encoder_right - current_encoder; // 离右限位的空间

        // 将编码器刻度转换为角度（GM6020是8192刻度/360度）
        float left_angle = (float)left_space * 360.0f / 8192.0f;   // 可以向左转的角度
        float right_angle = (float)right_space * 360.0f / 8192.0f; // 可以向右转的角度

        // 基于初始陀螺仪位置设置限位
        yaw_left_limit = left_angle;   // 左限位
        yaw_right_limit = -right_angle;  // 右限位

        // 初始化目标角度缓存
        if (vision_feedback->yaw != 0.0f || vision_feedback->pitch != 0.0f)
        {
            yaw_target_last = vision_feedback->yaw * hudu;
            pitch_target_last = vision_feedback->pitch * hudu;
        }
        else
        {
            // 首次运行且视觉无效，保持当前位置
            yaw_target_last = yaw_current;
            pitch_target_last = pitch_angle;
        }

        first_run = 0;
    }

    // 检测视觉数据是否有效
    uint8_t vision_valid = 1;

    // 判断条件：如果yaw和pitch都是0，认为是无效数据
    if (vision_feedback->yaw == 0.0f && vision_feedback->pitch == 0.0f)
    {
        vision_valid = 0;
    }

    if (vision_valid)
    {
        // 视觉有效：使用视觉发送的目标角度
        yaw_target = vision_feedback->yaw * hudu;

        // 对视觉目标进行机械限位保护
        if (yaw_target > yaw_left_limit)
        {
            yaw_target = yaw_left_limit;
        }
        else if (yaw_target < yaw_right_limit)
        {
          yaw_target = yaw_right_limit;
      }

      yaw_target_last = yaw_target;  // 更新缓存（已经限位后的值）
    }
    else
    {
        // 视觉丢失：使用上一次的有效目标（保持位置）
        yaw_target = yaw_target_last;
        // 注意：yaw_target_last 已经是限位后的值，无需再次限位
    }

    // 执行串级PID控制
    PID_CascadeCalc(motors[0].motor_data->pid, yaw_current, yaw_target, motors[0].motor_data->feedback->vel);
    motors[0].motor_data->target_position = motors[0].motor_data->pid[cascade_inner].Out;
    // ========== PITCH轴控制 ==========
    // 定义机械限位范围
    const float pitch_max_limit = pitch_max;    // PITCH轴最大值（向上限位）
    const float pitch_min_limit = pitch_min;    // PITCH轴最小值（向下限位）

    //float pitch_current = ins->Pitch;//pitch_angle;  // 获取当前PITCH角度
    float pitch_current = pitch_angle;
    // 确定PITCH目标角度
    float pitch_target;
    
    if (vision_valid)
    {
        // 视觉有效：使用视觉发送的目标角度
        pitch_target = vision_feedback->pitch * hudu;
        pitch_target_last = pitch_target;  // 更新缓存
    }
    else
    {
        // 视觉丢失：使用上一次的有效目标（保持位置）
        pitch_target = pitch_target_last;
    }
    
    // 首次运行处理（在上面已经处理，这里不再重复）
    if (first_run)
    {
        pitch_target = pitch_target_last;
    }
    
    // 机械限位保护：限制目标角度在允许范围内
    if (pitch_target > pitch_min_limit) 
    {
        pitch_target = pitch_min_limit;
    }
    else if (pitch_target < pitch_max_limit)
    {
        pitch_target = pitch_max_limit;
    }
    
    // 执行PITCH轴串级PID控制计算
    PID_CascadeCalc(motors[1].motor_data->pid, pitch_target, pitch_current, motors[1].motor_data->feedback->vel);
    
    motors[1].motor_data->target_position = motors[1].motor_data->pid[cascade_inner].Out;// + Gravity_compensation(ins->Pitch) * current_change;
}

// ============================================================================
// 数据测量函数实现
// ============================================================================

/**
 * @brief 云台数据测量函数
 * @param[out] measure_data 云台测量数据结构体指针
 * @param[in] ins 惯性导航系统数据指针
 * @param[in] motors 电机实例数组指针
 * @details 测量云台当前状态数据，传输给视觉系统使用
 */
void Gimbal_Measure(Gimbal_measure *measure_data, INS_t *ins, MotorInstance *motors)
{
    // 获取YAW轴角度和角速度
    measure_data->yaw = ins->Yaw;                                               // YAW轴角度
    measure_data->yaw_vel = motors[0].motor_data->feedback->vel * rad_change;   // YAW轴角速度（转换单位）
    
    // 获取PITCH轴角度和角速度
    measure_data->pitch  = ins->Pitch;                                          // PITCH轴角度
    measure_data->pitch_vel = motors[1].motor_data->feedback->vel * rad_change; // PITCH轴角速度（转换单位）
}

void Gimbal_PID_Change(MotorInstance *motors)
{
    static uint8_t last_auto_aim_flag = AUTO_AIM_OFF;
    // 检测PID是否需要切换
    if(auto_aim_flag != last_auto_aim_flag)
    {
        if(auto_aim_flag == AUTO_AIM_ON)
        {
            //视觉上次变量清零：
            yaw_target_last = 0;
            pitch_target_last = 0;
            PID_Clear(motors[0].motor_data->pid);
            PID_Clear(motors[1].motor_data->pid);
            // YAW轴（偏航轴）6020电机PID初始化
            PID_Init(&motors[0].motor_data->pid[single_loop], 1.0f, 0.0f, 0.0f, 10.0f, 6000.0f, 0.0f);
            PID_Init(&motors[0].motor_data->pid[cascade_inner], 70.0f, 0.0f, 0.0f, 600.0f, 7000.0f, 0.0f);
            PID_Init(&motors[0].motor_data->pid[cascade_outer], 7.0f, 0.0f, 0.0f, 10.0f, 55.0f, 0.0f);

            // PITCH轴（俯仰轴）6020电机PID初始化
            PID_Init(&motors[1].motor_data->pid[single_loop], 1.0f, 0.0f, 0.0f, 10.0f, 6000.0f, 0.0f);
            PID_Init(&motors[1].motor_data->pid[cascade_inner], 200.0f, 0.0f, 0.0f, 100.0f, 9000.0f, 0.0f);
            PID_Init(&motors[1].motor_data->pid[cascade_outer], 3.0f, 0.0f, 0.3f, 10.0f, 40.0f, 0.0f);

        }
        else

        {
            //视觉上次变量清零：
            yaw_target_last = 0;
            pitch_target_last = 0;
            PID_Clear(motors[0].motor_data->pid);
            PID_Clear(motors[1].motor_data->pid);
            // YAW轴（偏航轴）6020电机PID初始化
            PID_Init(&motors[0].motor_data->pid[single_loop], 1.0f, 0.0f, 0.0f, 10.0f, 6000.0f, 0.0f);
            PID_Init(&motors[0].motor_data->pid[cascade_inner], 90.0f, 5.0f, 0.0f, 600.0f, 7000.0f, 80.0f);
            PID_Init(&motors[0].motor_data->pid[cascade_outer], 25.0f, 0.4f, 0.0f, 10.0f, 55.0f, 0.0f);

            // PITCH轴（俯仰轴）6020电机PID初始化
            PID_Init(&motors[1].motor_data->pid[single_loop], 1.0f, 0.0f, 0.0f, 10.0f, 6000.0f, 0.0f);
            PID_Init(&motors[1].motor_data->pid[cascade_inner], 200.0f, 0.0f, 0.0f, 100.0f, 9000.0f, 0.0f);
            PID_Init(&motors[1].motor_data->pid[cascade_outer], 3.0f, 0.0f, 0.3f, 10.0f, 40.0f, 0.0f);
        }
        last_auto_aim_flag = auto_aim_flag;
    }
}
//pitch轴重力补偿(输出电流)
float Gravity_compensation(float pitch)
{
    float current = 0;
    current = 0.0001 * pitch * pitch - 0.0005 * pitch - 0.6315;
    return current;
}

/**
 * @brief 编码器值转换为PITCH角度
 * @param[in] motors PITCH轴电机实例指针
 * @return PITCH角度值（度）
 * @details 根据标定数据转换编码器值到角度：
 *          1913对应-17.7°（最上）
 *          3161对应36.5°（最下）
 *          转换系数：54.2° / 1248 = 0.04343°/单位
 */
float Get_Pitch_Angle_From_Encoder(MotorInstance *motors)
{
    // 参数检查
    if (motors == NULL || motors->motor_data == NULL || 
        motors->motor_data->feedback == NULL)
    {
        return 0.0f;
    }
    
    // 获取编码器原始值
    float encoder_pos = motors[1].motor_data->feedback->pos;
    
    // 转换为角度：offset为1913（对应-17.7度），系数为0.04343
    float pitch_angle = (encoder_pos - pitch_encoder_max) * 0.04343f + pitch_max;
    
    return pitch_angle;
}

/**
 * @brief 编码器值转换为PITCH角度
 * @param[in] motors PITCH轴电机实例指针
 * @return PITCH角度值（度）
 * @details 根据标定数据转换编码器值到角度：
 *          1913对应-17.7°（最上）
 *          3161对应36.5°（最下）
 *          转换系数：54.2° / 1248 = 0.04343°/单位
 */
float Get_Yaw_Angle_From_Encoder(MotorInstance *motors)
{
    // 参数检查
    if (motors == NULL || motors->motor_data == NULL || 
        motors->motor_data->feedback == NULL)
    {
        return 0.0f;
    }
    
    static uint8_t first_run = 1;
    float yaw_angle = 0.0f;
    static uint16_t first_encoder = 0.0f;
    if(first_run)
    {
        first_encoder = motors[0].motor_data->feedback->pos;
        first_run = 0;
    }
    
    // 获取当前编码器原始值
    uint16_t encoder_pos = motors->motor_data->feedback->pos;
    
    // 转换为角度：(初始位置 - 当前位置) * 转换系数
    // 这样使得：编码器减小（往左）-> 角度为正，编码器增大（往右）-> 角度为负
    yaw_angle = (first_encoder - encoder_pos) * (360.0f / 8192.0f);
    
    return yaw_angle;    
}

#endif // COMPILE_GIMBAL
