...
void referee_task(void *argument)
{
    for(;;)
    { 
        referee_usart_analyse(&referee_data);
 ...
 *
 */
void referee_usart_analyse(referee_receive *referee_data)
{
    ...
    switch (cmd_id)
    {
        ...
        case CMD_ID_POWER_HEAT:  // 0x0202: 实时底盘缓冲能量和射击热量数据
        {
            ...
            if (data_length == 14)
            {
                ...
                referee_data->power_heat.shooter_17mm_1_barrel_heat = frame_buffer[17] | (frame_buffer[18] << 8);  // buffer[19-20]
        ...
        case CMD_ID_GAME_STATUS:  // 0x0001: 比赛状态数据
        {
            if (data_length == 11)
            {
                game_status_data.game_type     = frame_buffer[7] & 0x0F;
                game_status_data.game_progress = (frame_buffer[7] >> 4) & 0x0F;
                game_status_data.stage_remain_time = frame_buffer[8] | (frame_buffer[9] << 8);
                memcpy(&game_status_data.SyncTimeStamp, &frame_buffer[10], 8);
            }
            break;
        }
        default:
