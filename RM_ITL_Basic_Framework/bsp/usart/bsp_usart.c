...
// UART5 遥控器接收包计数
volatile uint32_t sbus_rx_total = 0;  // 所有触发回调的包数（含丢帧）
volatile uint32_t sbus_rx_valid = 0;  // sbus_online == 1 时的有效包数
...
static volatile uint8_t usart8_tx_dma_done = 1;
static volatile uint8_t usart9_tx_dma_done = 1;
...
static usart_rx_callback_t usart8_user_callback = NULL;//usart8

uint8_t usart1_rx_buf[256];//串口1接收数据
uint8_t usart7_rx_buf[256];//串口2接收数据
uint8_t usart8_rx_buf[256];//串口2接收数据
uint8_t usart9_rx_buf[256];//串口1接收数据
...
//USART8
void bsp_usart8_set_callback(usart_rx_callback_t callback)
{
    usart8_user_callback = callback;
}
/**
...
uint8_t usart_tx_dma_send(UART_HandleTypeDef *huart, uint8_t *data, uint16_t len)
{
    ...
    if(huart->Instance == USART1)
        ...
        tx_done = &usart8_tx_dma_done;      
    else if(huart->Instance == UART9)
        tx_done = &usart9_tx_dma_done;          
...
void HAL_UART_TxCpltCallback(UART_HandleTypeDef *huart)
{
    if(huart->Instance == USART1)
        ...
        usart8_tx_dma_done = 1;        
    else if(huart->Instance == UART9)
        usart9_tx_dma_done = 1;          
    else if(huart->Instance == USART10)
...
{  
    ...
    else if(huart == &huart8)
    {
        ...
        HAL_UARTEx_ReceiveToIdle_DMA(&huart8, usart8_rx_buf, sizeof(usart8_rx_buf));
    }    
    else if(huart == &huart9)
    {
        HAL_UART_AbortReceive(&huart9);
        HAL_UARTEx_ReceiveToIdle_DMA(&huart9, usart9_rx_buf, sizeof(usart9_rx_buf));
    }    
...
void HAL_UARTEx_RxEventCallback(UART_HandleTypeDef *huart, uint16_t Size)
{
    ...
    else if(huart->Instance == UART5)
    {
        ...
        #endif
        if(sbus_online) sbus_rx_valid++;

        // 处理遥控器数据
        if(usart5_user_callback != NULL)
        {
            usart5_user_callback(sbus_buf, sizeof(sbus_buf));
        }
    ...
    else if(huart->Instance == UART7)
    {
        ...
        {
            usart7_user_callback(usart7_rx_buf, sizeof(usart7_rx_buf));
        }
    ...
    {
        // 处理舵机数据
        if(usart8_user_callback != NULL)
        {
            usart8_user_callback(usart8_rx_buf, Size);
        }
        usart_rx_dma_start(huart, usart8_rx_buf, sizeof(usart8_rx_buf));
    }    
    else if(huart->Instance == UART9)
    {
        usart_rx_dma_start(huart, usart9_rx_buf, sizeof(usart9_rx_buf));
    }
    // ... 其它串口同理
