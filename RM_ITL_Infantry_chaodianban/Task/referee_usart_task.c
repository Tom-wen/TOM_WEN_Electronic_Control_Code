...
void referee_usart_task(void const *argument)
{
    ...
    while (1)
    {
        ...
        if (draw_init_flag)//draw_init_flag
        {
            draw_init_all(send_id, receive_id);
            draw_init_flag = 0;
        }
        ...
        referee_unpack_fifo_data();
				//draw_shoot_accom(send_id, receive_id, 1);
        if (time_heart_ms == 5000)
        {
            time_heart_ms = 0;
        }
        else if (time_heart_ms % 700 == 0)
        {
            draw_shoot_accom(send_id, receive_id, 2);
        }
        else if (time_heart_ms % 550 == 0)
        {
            draw_state_of_top_circle(send_id, receive_id, 2);
        }
        else if(time_heart_ms%450==0)
        {
            draw_shoot_speed(send_id, receive_id, 2);
        }
        else if (time_heart_ms == 250)
        {
            draw_friction_wheel(send_id, receive_id, 2);
        }
        else if (time_heart_ms % 110 == 0)
        {
