...
void gimbal_task(void const *pvParameters)
{
  ...
  // 云台初始化
  gimbal_init();
  Upper_Computer_Init(&gimbal_control.gimbal_yaw_motor.absolute_angle);
  Upper_Computer_Init(&gimbal_control.gimbal_yaw_motor.absolute_angle_set);
...
static void gimbal_feedback_update(void)
{
  ...
  float pitch_rad = INS_data.angle_pitch;
  // 计算yaw轴角速度，考虑pitch角的影响
  gimbal_control.gimbal_yaw_motor.motor_gyro = -(arm_cos_f32(pitch_rad) * INS_data.wz
                                                 - arm_sin_f32(pitch_rad) * INS_data.wy);
}
